...
export function WhatsAppDialog({ title, content, action, open, onClose, ...other }) {
  ...
  const [folderListError, setFolderListError] = useState(false);
  ...
  const handleChangeCategoryList = useCallback((event) => {
    ...
    setFolderList(selectedValue); // Update the state with the selected value
    if (selectedValue) {
      setFolderListError(false); // Remove error if a value is selected
    }
  }, []);

  const handleAdd = () => {
    if (!folderList || folderList === 'None') {
      setFolderListError(true); // Set error if no valid folder is selected
    ...
    <>
      ...
      >
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            InputProps={{
              endAdornment: (
                <InputAdornment position="end">
                  <Tooltip title="Enter your Access Token here." arrow placement="top">
                    <Iconify
          ...
          />
          <Tooltip
            title="Click here to select the existing folder to store whatsapp number"
            arrow
            placement="top"
          >
            <TextField
              ...
              helperText={
                folderListError ? 'You must select a folder.' : 'Select folder name here.'
              }
              error={folderListError} // Add error prop based on validation
